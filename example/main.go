...
func main() {
	...
	if false {
		...
		txhash, err := con.Update(seller, []byte("01010100"),
	...
	if false {
		if true {
			...
			res, err := mp.GetPublishProductInfo(bs)
			fmt.Println(string(bs))
			fmt.Println(res)
			sdk.GetOntologySdk().NewRpcClient().SetAddress("http://172.168.3.47:20336")
			txhash, err := mp.Init(seller, dtoken, split)
			if err != nil {
				fmt.Println(err)
				return
			}
	...
	if false {
		...
		txHash, err := sdk.DefMpKit().Init(seller, dtokenContractAddr, splitContractAddr)
		if err != nil {
